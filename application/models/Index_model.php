...
class Index_model extends CI_Model{
    ...
    function uploadBase64Image($imgStr){
        ...
        $filename = $this->index_model->generateRef() . '.jpg';

        $file = $path . '/uploads/backend/'. $filename;
        $filePath = base_url('content/uploads/backend/'. $filename);
    ...
    function uploadDocument($fieldname='', $filename='', $uploadType='image', $dimensions=array()){
        $response = array();
        ...
        if($uploadType == 'image'){
            $config['upload_path'] = $this->gallery_path . '/uploads/backend/';
        }
    ...
    /**
     * Gets the last entry in a table
     * @param String $table Table to get last row
     * @returns Object Row object of the entry 
     */
    ...
    function sendEmail($to, $subject, $message, $attachment=''){
        ...
        $body = $this->load->view('emails/welcome.php');
        $settings = array(
            'to' => $to,
            'body' => $body,
            'subject' => "AAICE2019 Registration"
        );
        send_email($settings);
    }
    ...
    function  get_package_id($name){
        if($name){
            ...
            $this->error($this->CI->lang->line('error_no_package'));
        }
        return false;
    }

    /**
     * Updates user validity on email registration
     */
    function update_validity($email){
        $this->db->where('email',$email);
        $this->db->update('aauth_users',array('valid'=>1));
    }

    /**
     * Checks if user has registration details but cancelled so details present are invalid
     * Removes any found invalid details
     * TODO: To be reviewed
     */
    function delete_invalid_registration($email){
        $this->db->where('email',$email);
        $query1 = $this->db->get('aauth_users');
        if($query1->num_rows() === 1){
            $user_id = $query1->row()->id;
            if(!$query1->row()->valid){
                $this->deleteFromTable('aauth_users',array('id'=>$user_id));
                unlink(FCPATH.'content/uploads/registered_users/'.$query1->row()->picture);
                return true;
            }else{
                return false;
            }
    ...
    function unsubscribe($data){
        ...
        $this->db->where('service',$data['service']);
